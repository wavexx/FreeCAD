    ...
    for (int i = 0; i < getSize(); i++) {
        ...
        if(!writer.isForceXML()) {
            ...
            writer.Stream() << writer.ind() << " file=\""
                            << writer.addFile(getFileName(ext.c_str()), this)
                            << "\"/>\n";
        } else if(binary) {
...
bool PropertyTopoShapeList::isSame(const App::Property &_other) const
{
    if(!_other.isDerivedFrom(PropertyTopoShapeList::getClassTypeId()))
        return false;
    const auto &other = static_cast<const PropertyTopoShapeList&>(_other);
    if (other.getSize() != getSize())
        return false;
    for (int i=0, count=getSize(); i<count; ++i) {
        if (!_lValueList[i].isSame(other._lValueList[i]))
            return false;
    }
    return true;
}
