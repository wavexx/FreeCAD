...
struct MapperPrism: MapperMaker {
    std::vector<TopoShape> newEdges;
    std::vector<TopoDS_Shape> lastShapes;
    TopoShape uptoface;

    MapperPrism(BRepFeat_MakePrism &maker, const TopoShape &upto)
        :MapperMaker(maker), uptoface(upto)
    {
        std::vector<TopoShape> shapes;
        ...
        if (shapes.size()) {
            ...
            // And add all edges that shares a vertex with the profiles as new
            // edges.
            TopoShape profile;
            profile.makECompound(shapes, nullptr, false);
            TopoShape shape(maker.Shape());
            for (auto &vertex : profile.getSubShapes(TopAbs_VERTEX)) {
                for (auto &e : shape.findAncestorsShapes(vertex, TopAbs_EDGE)) {
                    if (profile.findShape(e))
                        continue;
                    ...
                    // BRepFeat_MakePrism::LastShape() is also useless for us. We'll
                    // search for the face that contains edge(s) that shares vertex of
                    // those new edges. And later on in generated() will'll map
                    // the given uptoface to the found last shapes.
                    auto otherVertex = TopExp::FirstVertex(TopoDS::Edge(e));
                    if (otherVertex.IsSame(vertex))
                        otherVertex = TopExp::LastVertex(TopoDS::Edge(e));
                    for (auto &e2 : shape.findAncestorsShapes(otherVertex, TopAbs_EDGE)) {
                        if (e2.IsSame(e))
                            continue;
                        for (auto &face : shape.findAncestorsShapes(e2, TopAbs_FACE)) {
                            if (!TopoShape(face).findShape(e)) {
                                bool found = false;
                                for (auto &s : lastShapes) {
                                    if (s.IsSame(face)) {
                                        found = true;
                                        break;
                                    }
        ...
        for(TopTools_ListIteratorOfListOfShape it(maker.NewEdges());it.More();it.Next())
            newEdges.push_back(it.Value());
    }
    virtual const std::vector<TopoDS_Shape> &generated(const TopoDS_Shape &s) const override {
        _res.clear();
        switch(s.ShapeType()) {
        case TopAbs_FACE:
            if (uptoface.findShape(s))
                return lastShapes;
            break;
        case TopAbs_VERTEX:
            for (auto &edge : newEdges) {
                if (edge.findShape(s)) {
                    _res.push_back(edge.getShape());
                    auto otherVertex = TopExp::FirstVertex(TopoDS::Edge(edge.getShape()));
                    if (otherVertex.IsSame(s))
                        otherVertex = TopExp::LastVertex(TopoDS::Edge(edge.getShape()));
                    _res.push_back(otherVertex);
                    return _res;
                }
            }
            break;
