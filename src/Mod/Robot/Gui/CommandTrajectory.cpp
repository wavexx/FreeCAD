...
void CmdRobotSetDefaultValues::activated(int)
{
    ...
    QString text = QInputDialog::getText(0, QObject::tr("Set default speed"),
                                          ...
                                          QStringLiteral("1 m/s"), &ok, Qt::MSWindowsFixedSizeDialogHint);
    if ( ok && !text.isEmpty() ) {
    ...
    text = QInputDialog::getText(0, QObject::tr("Set default acceleration"),
                                          ...
                                          QStringLiteral("1 m/s^2"), &ok, Qt::MSWindowsFixedSizeDialogHint);
    if ( ok && !text.isEmpty() ) {
        doCommand(Doc,"_DefAccelaration = '%s'",text.toUtf8().constData());
    }
