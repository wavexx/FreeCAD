    ...
    aliasOk(true)
{
    ...
    // Display unit
    connect(ui->displayUnit, &QLineEdit::textChanged, this, &PropertiesDialog::displayUnitChanged);
...
void PropertiesDialog::displayUnitChanged(const QString & text)
{
    ...
    try {
        auto expr = App::Expression::parseUnit(sheet, text.toUtf8().constData());
        auto e = static_cast<UnitExpression*>(expr.get());
