...
QString EditModeConstraintCoinManager::getPresentationString(const Constraint *constraint)
{
    ...
    */
    ...
    {
        if (constraintParameters.sDimensionalStringFormat.contains(QStringLiteral("%V")) ||
            constraintParameters.sDimensionalStringFormat.contains(QStringLiteral("%N")))
        {
                                            ...
                                            int *vPad)
{
    ...
    constraintName << type.toUtf8().data() << drawingParameters.constraintIconSize; // allow resizing by embedding size
    if (! Gui::BitmapFactory().findPixmapInCache(constraintName.str().c_str(), pxMap)) {
        pxMap = Gui::BitmapFactory().pixmapFromSvg(type.toUtf8().data(),QSizeF(drawingParameters.constraintIconSize, drawingParameters.constraintIconSize));
