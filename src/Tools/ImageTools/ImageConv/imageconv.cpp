...
bool CImageConvApp::Save(const QString& fn)
{
    ...
    if (!m_clPixmap.isNull())
    {
        if (!fn.isEmpty())
        {
            return m_clPixmap.save(fn, ext.toUpper().toUtf8());
        }
    }
...
bool CImageConvApp::ConvertToXPM(bool bAppendToFile)
{
    ...
    if (list.count() == 0)
    {
        cout << "Cannot find " << (const char*)m_Dir.nameFilters().join(" ").toUtf8() << endl;
        return false;
    }
    ...
    for (QStringList::Iterator it = list.begin(); it != list.end(); ++it)
    {
        ...
        cout << "Converting " << (const char*)(*it).toUtf8() << " ...";
...
{
    cerr << "Usage: " << (const char*)m_Executable.toUtf8() << " [OPTION(S)] -i input file(s) {-o output file}" << endl;
    cerr << "Try '"   << (const char*)m_Executable.toUtf8() << " --help' for more information." << endl;
...
{
    cerr << (const char*)m_Executable.toUtf8() << " 1.0.0 " << endl;
    exit(0);
}

void CImageConvApp::Usage()
{
    cerr << "Usage: " << (const char*)m_Executable.toUtf8() << " [OPTION(S)] -i input file(s) {-o output file}\n" << endl;
    cerr << "Options:" << endl;
    ...
    cerr << "This program supports the following image formats:\n"
         ...
         << (const char*)m_Executable.toUtf8() << " uses Qt Version " << qVersion() << "\n"
