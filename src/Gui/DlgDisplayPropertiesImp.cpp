...
public:
    ...
    static void setDrawStyle(const std::vector<Gui::ViewProvider*>& views, const char* property, QDoubleSpinBox* spinbox)
    {
    ...
    static void setPropertyBool(const std::vector<Gui::ViewProvider*>& views, const char *name, QCheckBox *checkbox)
    {
        bool enable = false;
        for (const auto & view : views) {
            if (auto prop = Base::freecad_dynamic_cast<App::PropertyBool>(view->getPropertyByName(name))) {
...
void DlgDisplayPropertiesImp::setupConnections()
{
    ...
    connect(d->ui.spinPointSize, qOverload<double>(&QDoubleSpinBox::valueChanged), this, &DlgDisplayPropertiesImp::onSpinPointSizeValueChanged);
    ...
    connect(d->ui.spinLineWidth, qOverload<double>(&QDoubleSpinBox::valueChanged), this, &DlgDisplayPropertiesImp::onSpinLineWidthValueChanged);
    ...
    connect(d->ui.checkBoxMapFaceColor, &QCheckBox::toggled, this, &DlgDisplayPropertiesImp::onMapFaceColorChanged);
    connect(d->ui.checkBoxMapLineColor, &QCheckBox::toggled, this, &DlgDisplayPropertiesImp::onMapLineColorChanged);
    connect(d->ui.checkBoxMapPointColor, &QCheckBox::toggled, this, &DlgDisplayPropertiesImp::onMapPointColorChanged);
    connect(d->ui.checkBoxMapTransparency, &QCheckBox::toggled, this, &DlgDisplayPropertiesImp::onMapTransparencyChanged);
}
