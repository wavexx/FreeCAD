  ...
  , ui(new Ui_DlgCustomActions)
{
    ...
    QString systemMacroDirStr = QString::fromUtf8(App::Application::getHomePath().c_str()) + QString::fromUtf8("Macro");
...
void DlgCustomActionsImp::on_buttonAddAction_clicked()
{
    ...
    if (!ui->actionAccel->text().isEmpty()) {
        ShortcutManager::instance()->setShortcut(
                actionName.constData(), ui->actionAccel->text().toUtf8().constData());
    }
...
void DlgCustomActionsImp::on_buttonReplaceAction_clicked()
{
    ...
    if (!ui->actionAccel->text().isEmpty()) {
        macro->setAccel(ui->actionAccel->text().toUtf8());
    }
    ui->actionAccel->clear();
        ...
        for (const auto & path : paths) {
            if (search.indexOf(path) < 0) {
                ...
                for (const auto & format : formats)
                    filters << QStringLiteral("*.%1").arg(QString::fromUtf8(format).toLower());
                QDir d(path);
...
QString DlgCustomActionsImp::newActionName()
{
    ...
    {
        bUsed = false;
        sName = (ui->checkBoxPreselect->isChecked() ?
                 QStringLiteral("Std_Macro_Presel_%1") : 
                 QStringLiteral("Std_Macro_%1")).arg( id++ );

        for (const auto &it : aclCurMacros) {
            if (sName == QString::fromUtf8(it->getName()))
            {
                bUsed = true;
                break;
            }
