...
{
    return getMDIViewPyObject();
}

MDIViewPy* MDIView::getMDIViewPyObject()
{
    if (!mdiViewPy)
        mdiViewPy = new MDIViewPy(this);

    Py_INCREF(mdiViewPy);
    return mdiViewPy;
}

MDIViewPy *MDIView::castFrom(PyObject *pyObj)
{
    if (PyObject_TypeCheck(pyObj, &MDIViewPy::Type)) {
        Py_INCREF(pyObj);
        return static_cast<MDIViewPy*>(pyObj);
    }
    try {
        auto res = Py::Object(pyObj).callMemberFunction("cast_to_base");
        if (PyObject_TypeCheck(res.ptr(), &MDIViewPy::Type)) {
            return static_cast<MDIViewPy*>(res.ptr());
        }
    } catch (Py::Exception &)
    {
    }
    throw Py::TypeError("Failed to cast to MDIViewPy");
}
