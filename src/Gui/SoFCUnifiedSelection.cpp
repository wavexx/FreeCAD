...
{
    this->highlightMode = ViewParams::getEnablePreselection()?ON:OFF;
    this->selectionMode = ViewParams::getEnableSelection()?ON:OFF;
    ...
    for(int i=0,count=points.getLength();i<count;++i) {
        ...
        if(!(master->useNewSelection.getValue()
            ...
            || (!info.vpd->isSelectable()
                && !ViewParams::getOverrideSelectability()))
        {
            if(!singlePick) continue;
            if(ret.empty()) {
                info.vpd = nullptr;
...
bool SoFCUnifiedSelection::Private::doAction(SoAction * action)
{
                ...
                && (vp->isSelectable() || ViewParams::getOverrideSelectability()))
            {
                ...
                if(vp->getDetailPath(hilaction->SelChange->pSubName,detailPath,true,det)) {
                    setHighlight(detailPath,det,static_cast<ViewProviderDocumentObject*>(vp),
                                 ...
                                 hilaction->SelChange->SubType == SelectionChanges::MsgSource::TreeView);
                }
                delete det;
            }
        }
    ...
    if (action->getTypeId() == SoFCSelectionAction::getClassTypeId()) {
                    ...
                    && selaction->SelChange->Type == SelectionChanges::SetSelection))
        {
            ...
            if (selaction->SelChange->Type == SelectionChanges::SetSelection && this->pcDocument) {
                auto sels = Selection().getSelectionT(this->pcDocument->getDocument()->getName(), ResolveMode::NoResolve);
                                            ...
                                            bool altDown) 
{
    ...
    if (ctrlDown) {
        if(Gui::Selection().isSelected(docname,objname,info.subname.c_str(),ResolveMode::NoResolve))
...
{
    bool res = false;
    auto mymode = static_cast<HighlightModes>(master->highlightMode.getValue());
    ...
    if (doPick || event->isOfType(SoLocation2Event::getClassTypeId())) {
        ...
        if (mymode == AUTO || mymode == ON) {
            res = true;
            double delay = ViewParams::getPreSelectionDelay();
...
bool SoFCUnifiedSelection::getShowSelectionBoundingBox() {
    return ViewParams::getShowSelectionBoundingBox()
        || _ShowBoundBox;
}
...
SoSelectionElementAction::SoSelectionElementAction (Type t, bool secondary, bool noTouch)
    : _type(t), _det(nullptr), _secondary(secondary), _noTouch(noTouch)
{
        ...
        const SbColor &color, const SbMatrix *mat, bool force)
{
    ...
    bool project = !mat && ViewParams::getRenderProjectedBBox();
        ...
        const SbBox3f &bbox, SbColor color, const SbMatrix *mat)
{
    ...
    } else if(ViewParams::getRenderProjectedBBox()) {
    ...
    SoDrawStyleElement::set(state,SoDrawStyleElement::LINES);
    SoLineWidthElement::set(state,ViewParams::getSelectionBBoxLineWidth());
...
bool SoFCSelectionRoot::_renderPrivate(SoGLRenderAction * action, bool inPath, bool &pushed) {
       ...
       && ctx && !ctx->hideAll && (ctx->selAll || ctx->hlAll))
    {
        if (style==SoFCSelectionRoot::PassThrough) {
            ...
            if(_ShowBoundBox || !ViewParams::getShowSelectionOnTop()) {
                SoCacheElement::invalidate(state);
                if(ViewParams::getUseTightBoundingBox() && viewProvider) {
                    Base::Matrix4D mat;
                    bool project = ViewParams::getRenderProjectedBBox();
                    if(project)
...
void SoFCSelectionRoot::rayPick(SoRayPickAction * action) {
    if(!ViewParams::getOverrideSelectability()
    ...
    if (action->isRenderingDelayedPaths()) {
        ...
        if(det)
            ...
            if(!bbox)
                ...
                if(!sel && !hl)
                    selColor.setPackedValue(ViewParams::getSelectionColor(),trans);
                ...
                if(!viewProvider || det) {
                    ...
                    if(ViewParams::getRenderProjectedBBox()) {
                        if(!ViewParams::getUseTightBoundingBox())
                            SoFCSelectionRoot::renderBBox(action,this,hl?hlColor:selColor);
                        else {
