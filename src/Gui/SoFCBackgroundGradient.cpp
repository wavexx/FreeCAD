...
void SoFCBackgroundGradient::GLRender (SoGLRenderAction * /*action*/)
{
    ...
    if (gradient == Gradient::LINEAR) {
        ...
        glColor3f(fCol[0], fCol[1], fCol[2]); glVertex3f(0.0f, 0.0f, -1);

        if (mCol[0] < 0) { // simple radial gradient
            glColor3f(tCol[0], tCol[1], tCol[2]);
            for (const GLfloat *vertex : big_circle)
                glVertex3f( vertex[0], vertex[1], -1 );
            glVertex3f( big_circle.front()[0], big_circle.front()[1], -1 );
        } else {
            glColor3f(mCol[0], mCol[1], mCol[2]);
            for (const GLfloat *vertex : small_oval)
                glVertex3f( vertex[0], vertex[1], -1 );
            glVertex3f( small_oval.front()[0], small_oval.front()[1], -1 );
            glEnd();

            glBegin(GL_TRIANGLE_STRIP);
            for (std::size_t i = 0; i < small_oval.size(); i++) {
                glColor3f(mCol[0], mCol[1], mCol[2]); glVertex3f( small_oval[i][0], small_oval[i][1], -1 );
                glColor3f(tCol[0], tCol[1], tCol[2]); glVertex3f( big_circle[i][0], big_circle[i][1], -1 );
            }

            glColor3f(mCol[0], mCol[1], mCol[2]); glVertex3f( small_oval.front()[0], small_oval.front()[1], -1 );
            glColor3f(tCol[0], tCol[1], tCol[2]); glVertex3f( big_circle.front()[0], big_circle.front()[1], -1 );
        }
