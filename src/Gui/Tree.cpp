...
QSize TreeWidgetItemDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    int spacing = std::max(0, (int)TreeParams::getItemSpacing());
    ...
    , myName(name)
{
    ...
    if (TreeParams::getFontSize() > 0) {
        ...
        font.setPointSize(TreeParams::getFontSize());
        this->setFont(font);
    }
    ...
    connect(this->header(), &QHeaderView::sectionResized,
            this, &TreeWidget::onColumnResized);
    ...
    connect(this, &QTreeWidget::pressed, this, &TreeWidget::onItemPressed);
    connect(this->toolTipTimer, &QTimer::timeout, this, &TreeWidget::onToolTipTimer);
    preselectTime.start();
...
void TreeWidget::itemSearch(const QString &text, bool select) {
    ...
    try {
        ...
        if(select || TreeParams::getSyncView()) {
            Gui::Selection().clearSelection();
...
void TreeWidget::onMarkRecompute()
{
    ...
    // mark all selected objects
        ...
        for (QList<QTreeWidgetItem*>::const_iterator it = items.cbegin(); it != items.cend(); ++it) {
...
{
    return TreeParams::getItemSpacing();
}
                                 ...
                                 App::SubObjectT *ctxObj)
{
    ...
    if(!sobj) {
        ...
        if (sels.empty()) {
            sels = Gui::Selection().getSelectionT(nullptr, ResolveMode::NoResolve, true);
            if (sels.empty())
                return false;
        }
        sobj = &sels.front();
...
bool TreeWidget::onDoubleClickItem(QTreeWidgetItem *item)
{
    try {
        if (item->type() == TreeWidget::DocumentType) {
            auto doc = static_cast<DocumentItem*>(item)->document();
            if (!doc)
                return false;
...
void TreeWidget::dropEvent(QDropEvent *event)
{
    ...
    if (targetItem->type() == TreeWidget::ObjectType) {
        ...
        auto sels = Selection().getSelectionT(nullptr, ResolveMode::NoResolve);
        Selection().selStackPush();
...
void TreeWidget::scrollItemToTop()
{
    ...
    auto tree = instance();
    if (tree && tree->isSelectionAttached() && !tree->isSelectionBlocked()) {
...
void TreeWidget::onItemSelectionChanged ()
{
    ...
    if(selItems.size()<=1) {
        ...
        Selection().clearCompleteSelection();
        DocumentObjectItem *item=nullptr;
        App::DocumentObject *preselObj=nullptr;
        ...
        if(selItems.size()) {
            if(selItems.front()->type() == ObjectType) {
                ...
                if(!ViewParams::getShowSelectionOnTop()) {
                    ...
                    item->getSubName(ss,topParent);
                    if(topParent) {
                        if(!obj->redirectSubName(ss,topParent,nullptr))
                            ss << obj->getNameInDocument() << '.';
                        obj = topParent;
                    }
                    auto subname = ss.str();
        ...
        if(preselObj) {
            ...
            Selection().setPreselect(preselObj->getDocument()->getName(),
                    preselObj->getNameInDocument(),preselSub.c_str(),0,0,0,
                    SelectionChanges::MsgSource::TreeView);
        }
    }else{
        for (auto pos = DocumentMap.begin();pos!=DocumentMap.end();++pos)
...
void TreeWidget::onSelectTimer() {
    ...
    bool syncSelect = instance()==this && TreeParams::getSyncSelection();
...
void DocumentItem::slotInEdit(const Gui::ViewProviderDocumentObject& v)
{
    ...
    unsigned long col = TreeParams::getTreeEditColor();
...
void DocumentItem::selectItems(SelectionReason reason) {
    ...
    bool sync;
    if (ViewParams::getMaxOnTopSelections()<(int)sels.size() || reason==SR_SELECT)
        sync = false;
    else
        sync = true;
    ...
    DocumentObjectItem *newCurrent = nullptr;
...
void DocumentObjectItem::setHighlight(bool set, Gui::HighlightMode high) {
    ...
    {
        f.setBold(TreeParams::getTreeActiveBold());
        f.setItalic(TreeParams::getTreeActiveItalic());
        f.setUnderline(TreeParams::getTreeActiveUnderlined());
        f.setOverline(TreeParams::getTreeActiveOverlined());
